...
const EachRequest = ({ mongouser, users }) => {
    ...
    return (
      <div className="bg-base-200 p-4 rounded flex flex-col justify-between items-center md:flex-row gap-3 relative">
        ...
        <div className="md:ml-10 lg:ml-16">
          {!clicked ? (
            <>
              <button
                onClick={() => handleRequest(mongouser)}
                className="btn md:rounded-full w-full "
              >
            ...
            <>
              <button className="btn btn-disabled md:rounded-full w-full">
                Request Sent
              </button>
            </>
          )}
